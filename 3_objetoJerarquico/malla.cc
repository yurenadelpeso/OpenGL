...
GLuint CrearVBO( GLuint tipo_vbo, GLuint tamanio_bytes, GLvoid *puntero_ram){
  ...
  glBufferData( tipo_vbo, tamanio_bytes, puntero_ram, GL_STATIC_DRAW); //esta instruccion hace la transferenvia de datos desde la RAM hacia la GPU
...
void ObjMallaIndexada::draw_ModoDiferido()
{
  ...
  if(id_VboVertices==0) {
    id_VboVertices=CrearVBO(GL_ARRAY_BUFFER,3*vertices.size()*sizeof(float),vertices.data());
  }
  if(id_VboTriangulos==0) {
    id_VboTriangulos=CrearVBO(GL_ELEMENT_ARRAY_BUFFER,3*triangulos.size()*sizeof(unsigned int),triangulos.data());
  }
   if(id_VboColor==0) {
    id_VboColor=CrearVBO(GL_ELEMENT_ARRAY_BUFFER,3*triangulos.size()*sizeof(unsigned int), color.data());
  }
...
void ObjMallaIndexada::ajedrezado(){
  ...
  // indicar el formato y la dirección de memoria del array de vértice (son tuplas de 3 valores float, sin espacio entre ellas)  
...
// -----------------------------------------------------------------------------
// Función de visualización de la malla puede llamar a  draw_ModoInmediato o bien a draw_ModoDiferido
void ObjMallaIndexada::draw(int modo, bool ajedrez)
{
  if(ajedrez){
     ajedrezado();
  }
...
Cupula::Cupula(const int num_vert_perfil, const int num_instancias_perf){
  std::cout<<"CONSTRUCTOR DE LA ESFERA";
...
ObjRevolucion::ObjRevolucion( const std::string & nombre_ply_perfil , int nrev)
{
  ...
  if(nrev!=0) crearMalla(vertices, nrev);
   color.clear();
  for(int i=0;i<triangulos.size();i++){
        color.push_back({0.4,0.4,0.4});
    }
    ...
    {for (int i=0;i<num_aux;i++) //for de puntos de la forma
       {
        ...
        x=perfil_original[i][0]*cos(2.0*M_PI*j/(1.0*num_instancias_perfil))+perfil_original[i][2]*sin(2.0*M_PI*j/(1.0*num_instancias_perfil));
        //std::cout<<"CALCULO X : "<<x;std::cout<<"\n";
        y=perfil_original[i][1];
        //std::cout<<"CALCULO Y : "<<y;std::cout<<"\n";
        z=-perfil_original[i][0]*sin(2.0*M_PI*j/(1.0*num_instancias_perfil))+perfil_original[i][2]*cos(2.0*M_PI*j/(1.0*num_instancias_perfil));
        //std::cout<<"CALCULO Z : "<<z;std::cout<<"\n";
...
std::cout<<"PREPARANDO CARAS\n";
  for (int j=0;j<num_instancias_perfil;j++){ //j para los perfiles
    for (int i=0;i<num_aux-1;i++){ //i para los puntos
      x_=i+((j+1)%num_instancias_perfil)*num_aux; //el modulo de num se usa para crear la ultima cara (enganchar ultimo perfil con el primero)
      //Forma las caras triangulo de arriba
      
      //std::cout<<"calculando cara numero "<<i;std::cout<<"valor de la cordena da X : "<<x_;std::cout<<"\n";
