...
// Generación puntos del perfil ")"
  ...
  //crearMallaSinTapas(radio_base, num_instancias_perf)
  ...
  float x=0,y=0,z=0;
    ...
    for (int j=0;j<num_instancias_perfil;j++){ //j para los perfiles
      for (int i=0;i<num_aux-1;i++){ //i para los puntos
        ...
        //Forma las caras triangulo de arriba   // std::cout<<"calculando cara numero "<<i;std::cout<<"valor de la cordena da X : "<<x_; std::cout<<"\n";
        ...
        //Forma las caras triangulo de abajo
          x_=i+1+j*num_aux;// std::cout<<"cara numero "<<i; std::cout<<"valor de la cordena da X: "<<x_; std::cout<<"\n";
...
std::cout<<"TAPA INFERIOR\n";
  ...
  if (fabs(perfil_original[num_aux-1][0])>0.0) // me quedaria con el punto final del perfil
  {
    if(perfil_original[num_aux -1][1]==-altura)y=altura;
      else y=perfil_original[num_aux-1][1];// defino el punto de arriba del cono
    x=0;
    z=0;
    vertices.push_back({x,y,z});
